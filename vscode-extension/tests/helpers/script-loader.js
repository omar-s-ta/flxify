/**
 * Script loader for VS Code extension tests — replicates the metadata parsing
 * and module loading from extension.ts so tests can run scripts the same way
 * the extension does.
 */
const fs = require('fs');
...
/**
 * Creates a require shim that loads lib modules from src/scripts/lib/
 ...
 * Replicates the extension.ts parsing logic including trailing comma cleanup.
 */
 ...
 * The execute function takes (requireShim, state) just like in extension.ts.
 */
...
/**
 * Returns an array of all script filenames in src/scripts/.
 */
